...
function renderAuth(){
  root.innerHTML=`<main class="auth-page"><div class="auth-card">
    ...
    <div id="registerBox" class="auth-form" style="display:none">
      ...
      <div id="authMessage"></div><button class="primary" onclick="register()">Crea il mio account</button><button class="secondary" onclick="showLogin()">Ho già un account</button>
    </div>
  </div></main>`;
  pickRole("reader");
}
...
function showAuthMessage(message,type="error"){ const box=document.querySelector("#authMessage"); if(box) box.innerHTML=message?`<div class="inline-message ${type}">${escapeHTML(message)}</div>`:""; }
...
async function login(){
  showAuthMessage(""); const identifier=document.getElementById("identifier").value.trim(), password=document.getElementById("password").value;
  if(!identifier||!password){ showAuthMessage("Inserisci email/username e password."); return; }
...
async function register(){
  showAuthMessage(""); const displayName=document.getElementById("rname").value.trim(), username=document.getElementById("ruser").value.trim(), email=document.getElementById("remail").value.trim(), password=document.getElementById("rpass").value;
  if(!displayName||!username||!email||!password){ showAuthMessage("Completa tutti i campi per creare l'account."); return; }
  if(password.length<8){ showAuthMessage("La password deve avere almeno 8 caratteri."); return; }
...
function renderProfileHeader(){
  ...
  const stats=me.stats||{}; el.innerHTML=`<div class="profile-header"><div class="profile-top"><div class="profile-avatar">${initials(me.display_name)}</div><div class="profile-info"><span class="eyebrow">${me.role==="writer"?"SCRITTORE":"LETTORE"}</span><h1>${escapeHTML(me.display_name)}</h1><p>@${escapeHTML(me.username)}</p></div><button class="secondary compact" onclick="logout()">Esci</button></div><p class="profile-bio">${escapeHTML(me.bio||"Scrivo ciò che non riesco a dire.")}</p><div class="profile-stats"><div><strong>${stats.poems||0}</strong><span>Poesie</span></div><div><strong>${stats.followers||0}</strong><span>Follower</span></div><div><strong>${stats.following||0}</strong><span>Seguiti</span></div></div></div>`;
}
