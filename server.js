...
function canViewPoem(p, viewerId) {
  ...
  return p.visibility === "followers" && isFollowing(viewerId, p.user_id);
}
function poem(id, viewerId = 0) {
  const p = db.prepare(`
    SELECT p.*, u.username, u.display_name, u.role,
      (SELECT COUNT(*) FROM likes WHERE poem_id=p.id) AS likes,
      (SELECT COUNT(*) FROM saves WHERE poem_id=p.id) AS saves,
      (SELECT COUNT(*) FROM comments WHERE poem_id=p.id) AS comments
    FROM poems p JOIN users u ON u.id=p.user_id WHERE p.id=?
  `).get(id);
  if (!p || !canViewPoem(p, viewerId)) return null;
  return {
    ...p,
    liked: !!db.prepare("SELECT 1 FROM likes WHERE user_id=? AND poem_id=?").get(viewerId, id),
    saved: !!db.prepare("SELECT 1 FROM saves WHERE user_id=? AND poem_id=?").get(viewerId, id),
    following: isFollowing(viewerId, p.user_id)
  };
}
function userStats(userId) {
  return {
    poems: db.prepare("SELECT COUNT(*) n FROM poems WHERE user_id=?").get(userId).n,
    followers: db.prepare("SELECT COUNT(*) n FROM follows WHERE following_id=?").get(userId).n,
    following: db.prepare("SELECT COUNT(*) n FROM follows WHERE follower_id=?").get(userId).n,
    likes: db.prepare("SELECT COUNT(*) n FROM likes l JOIN poems p ON p.id=l.poem_id WHERE p.user_id=?").get(userId).n
  };
}
function profilePayload(userId, viewerId) {
  ...
  const ids = db.prepare(own
    ? "SELECT id FROM poems WHERE user_id=? ORDER BY created_at DESC"
    : "SELECT id FROM poems WHERE user_id=? AND visibility='public' ORDER BY created_at DESC"
...
app.post("/api/register", (req, res, next) => {
  try {
    ...
    if (db.prepare("SELECT 1 FROM users WHERE username=?").get(username)) return res.status(409).json({ error: "Questo username è già utilizzato." });
    if (db.prepare("SELECT 1 FROM users WHERE email=?").get(email)) return res.status(409).json({ error: "Questa email è già associata a un account." });
    const hash = bcrypt.hashSync(password, 12);
    const r = db.prepare("INSERT INTO users(username,email,password,display_name,role) VALUES(?,?,?,?,?)").run(username,email,hash,displayName,role);
...
app.post("/api/login", (req, res) => {
  ...
  const u = db.prepare("SELECT * FROM users WHERE email=? OR username=?").get(identifier, identifier);
  if (!u || !bcrypt.compareSync(password, u.password)) return res.status(401).json({ error: "Email/username o password non corretti." });
  const token = jwt.sign({ id: u.id, username: u.username }, SECRET, { expiresIn: "7d" });
  res.json({ ok: true, token, user: publicUser(u.id) });
});
...
app.get("/api/poems", auth, (req, res) => {
  ...
  const ids = db.prepare(`
    ...
    WHERE (
      ...
      p.user_id=? OR
      (p.visibility='followers' AND EXISTS(SELECT 1 FROM follows f WHERE f.follower_id=? AND f.following_id=p.user_id))
    )
    AND (?='' OR p.mood=?)
    AND (?='' OR lower(p.title||' '||p.body||' '||u.display_name||' '||u.username) LIKE lower(?))
    ORDER BY p.created_at DESC LIMIT 100
  `).all(req.user.id, req.user.id, mood, mood, q, `%${q}%`).map(x => x.id);
  res.json(ids.map(id => poem(id, req.user.id)).filter(Boolean));
});
...
app.post("/api/poems", auth, (req, res, next) => {
  try {
    ...
    const r = db.prepare("INSERT INTO poems(user_id,title,body,mood,visibility) VALUES(?,?,?,?,?)").run(req.user.id,title,body,mood,visibility);
    res.status(201).json({ ok: true, message: "Poesia pubblicata.", poem: poem(r.lastInsertRowid, req.user.id) });
  } catch (e) { next(e); }
});

app.get("/api/poems/:id", auth, (req, res) => {
  const p = poem(req.params.id, req.user.id);
  p ? res.json(p) : res.status(404).json({ error: "Poesia non trovata o non disponibile." });
});
...
app.delete("/api/poems/:id", auth, (req, res, next) => {
  try {
    ...
    if (p.user_id !== req.user.id) return res.status(403).json({ error: "Puoi eliminare solo le tue poesie." });
...
function toggle(table, userId, poemId) {
  const p = db.prepare("SELECT * FROM poems WHERE id=?").get(poemId);
  if (!p || !canViewPoem(p, userId)) return { error: "Poesia non disponibile." };
  ...
  return { poem: poem(poemId, userId) };
}

app.post("/api/poems/:id/like", auth, (req,res) => {
  const result = toggle("likes", req.user.id, req.params.id);
  if (result.error) return res.status(404).json(result);
  res.json({ ok: true, ...result });
});
app.post("/api/poems/:id/save", auth, (req,res) => {
  const result = toggle("saves", req.user.id, req.params.id);
  if (result.error) return res.status(404).json(result);
  res.json({ ok: true, ...result });
});
...
app.post("/api/users/:id/follow", auth, (req,res) => {
  ...
  if (!publicUser(target)) return res.status(404).json({ error: "Utente non trovato." });
  const exists = isFollowing(req.user.id,target);
  if (exists) db.prepare("DELETE FROM follows WHERE follower_id=? AND following_id=?").run(req.user.id,target);
  else {
...
app.get("/api/poems/:id/comments", auth, (req,res) => {
  if (!poem(req.params.id, req.user.id)) return res.status(404).json({ error:"Poesia non disponibile." });
  res.json(db.prepare(`SELECT c.id,c.body,c.created_at,u.username,u.display_name FROM comments c JOIN users u ON u.id=c.user_id WHERE c.poem_id=? ORDER BY c.created_at ASC`).all(req.params.id));
});
app.post("/api/poems/:id/comments", auth, (req,res,next) => {
  try {
    const p = db.prepare("SELECT * FROM poems WHERE id=?").get(req.params.id);
    if (!p || !canViewPoem(p,req.user.id)) return res.status(404).json({ error:"Poesia non disponibile." });
    const body = String(req.body.body || "").trim();
    ...
    if (p.user_id !== req.user.id) db.prepare("INSERT INTO notifications(user_id,actor_id,type,poem_id) VALUES(?,?,?,?)").run(p.user_id,req.user.id,"comment",req.params.id);
    res.status(201).json({ ok:true, comment:db.prepare(`SELECT c.id,c.body,c.created_at,u.username,u.display_name FROM comments c JOIN users u ON u.id=c.user_id WHERE c.id=?`).get(r.lastInsertRowid) });
  } catch(e) { next(e); }
});

app.get("/api/notifications", auth, (req,res) => res.json(db.prepare(`SELECT n.*,u.username,u.display_name FROM notifications n LEFT JOIN users u ON u.id=n.actor_id WHERE n.user_id=? ORDER BY n.created_at DESC LIMIT 50`).all(req.user.id)));
...
app.get("/api/users/:username", auth, (req,res) => {
  ...
  res.json(profilePayload(u.id, req.user.id));
});

app.get("/api/profile", auth, (req,res) => res.json(profilePayload(req.user.id, req.user.id)));
...
app.listen(PORT,"0.0.0.0",() => console.log(`VERSI avviato sulla porta ${PORT}`));
