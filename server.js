...
const SECRET =
  ...
  "cambia-questa-chiave-in-produzione";
...
function auth(req, res, next) {
  ...
  const value = raw.startsWith("Bearer ")
    ? raw.slice(7)
    : "";
  ...
  (req, res, next) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Lo username deve contenere 3-24 caratteri: lettere, numeri, punto, trattino o underscore."
        });
      }
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Scegli se vuoi essere Scrittore o Lettore."
        });
      }
    ...
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error:
            "Solo gli Scrittori possono pubblicare poesie."
        });
      }
...
app.get(
  ...
  (req, res) => {
    ...
    if (!p) {
      return res.status(404).json({
        error:
          "Poesia non trovata o non disponibile."
      });
    }
  ...
  (req, res, next) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error:
            "Puoi eliminare solo le tue poesie."
        });
      }
  ...
  (req, res, next) => {
    try {
      ...
      if (name.length > 60) {
        return res.status(400).json({
          error:
            "Il nome della raccolta può contenere al massimo 60 caratteri."
        });
      }
  ...
  (req, res, next) => {
    try {
      ...
      if (exists) {
        return res.status(409).json({
          error:
            "Questa poesia è già nella raccolta."
        });
      }
...
app.use(
  ...
  (error, req, res, next) => {
    ...
    res.status(500).json({
      error:
        "Abbiamo avuto un problema temporaneo. Riprova tra poco."
    });
  }
);
